function [C,frq,wave] = conv2_gaus(dem,a,dx)

%[C,frq,wave] = conv2_gaus(dem,a,dx);
...
% Revised from conv2_mexh to conv2_gaus by W.T Struble (06/2019)
...
psi =  (1/(2*pi*(a*dx)^2)).*exp(-(X.^2 + Y.^2)/(2*a^2));
...
wave = 2*pi*dx*a/(1/2)^(1/2);      %Torrence and Compo (1998)
frq = 1./wave;
